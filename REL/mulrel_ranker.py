...
class PreRank(torch.nn.Module):
    ...
    def forward(self, token_ids, token_offsets, entity_ids, embeddings, emb):
        ...
        sent_vecs = embeddings['word_embeddings_bag'](token_ids, token_offsets)  # (batch_size, emb_size=300)

        # entity_vecs = emb.emb(entity_names)

        entity_vecs = embeddings['entity_embeddings'](entity_ids)  # (batch_size, n_cands, emb_size)
...
class MulRelRanker(torch.nn.Module):
    def __init__(self, config, device):
        ...
        self.tok_score_mat_diag = torch.nn.Parameter(torch.ones(self.config['emb_dims']))
        ...
        if self.config['use_local']:
            self.ent_localctx_comp = torch.nn.Parameter(torch.ones(self.config['emb_dims']))

        if self.config['use_pad_ent']:
            self.pad_ent_emb = torch.nn.Parameter(torch.randn(1, self.config['emb_dims']) * 0.1)
            self.pad_ctx_vec = torch.nn.Parameter(torch.randn(1, self.config['emb_dims']) * 0.1)
        ...
        self.rel_embs = torch.randn(self.config['n_rels'], self.config['emb_dims']) * 0.01
    ...
    def __local_ent_scores(self, token_ids, tok_mask, entity_ids, entity_mask, embeddings, p_e_m=None):
        ...
        selected_tok_vecs = torch.gather(
            tok_vecs, dim=1, index=top_tok_att_ids.view(batchsize, -1, 1).repeat(1, 1, tok_vecs.size(2)),
        )
    ...
    def forward(self, token_ids, tok_mask, entity_ids, entity_mask, p_e_m, embeddings, gold=None):
        """
        ...
        if self.config['use_local']:
            ...
            local_ent_scores = Variable(torch.zeros(n_ments, n_cands), requires_grad=False).to(self.device)

        # compute context vectors
        ltok_vecs = embeddings['snd_embeddings'](self.s_ltoken_ids) * self.s_ltoken_mask.view(n_ments, -1, 1)
        local_lctx_vecs = torch.sum(ltok_vecs, dim=1) / torch.sum(self.s_ltoken_mask, dim=1, keepdim=True).add_(1e-5)
        rtok_vecs = embeddings['snd_embeddings'](self.s_rtoken_ids) * self.s_rtoken_mask.view(n_ments, -1, 1)
        local_rctx_vecs = torch.sum(rtok_vecs, dim=1) / torch.sum(self.s_rtoken_mask, dim=1, keepdim=True).add_(1e-5)
        mtok_vecs = embeddings['snd_embeddings'](self.s_mtoken_ids) * self.s_mtoken_mask.view(n_ments, -1, 1)
        ...
        if self.config['use_pad_ent']:
            ...
            local_ent_scores = torch.cat(
                [local_ent_scores, Variable(torch.zeros(1, n_cands), requires_grad=False).to(self.device),], dim=0,
            )
            n_ments += 1
        ...
        if n_ments == 1:
            ...
            rel_ctx_ctx_scores = rel_ctx_ctx_scores.add_((1 - Variable(dist.float()).to(self.device)).mul_(-1e10))
            ...
            ent_ent_scores = torch.sum(
                rel_ent_ent_scores * rel_ctx_ctx_weights.view(n_rels, n_ments, 1, n_ments, 1), dim=0,
            ).mul(
                1.0 / n_rels
            ...
            for _ in range(self.config['n_loops']):
                ...
                ent_ent_votes = (
                    ...
                    + local_ent_scores * 1
                    + torch.sum(
                        prev_msgs.view(1, n_ments, n_cands, n_ments) * mask.view(n_ments, 1, 1, n_ments), dim=3,
    ...
    def loss(self, scores, true_pos, lamb=1e-7):
        ...
        diff = (
            (X.view(self.config['n_rels'], 1, -1) - X.view(1, self.config['n_rels'], -1))
            .pow(2)
            .sum(dim=2)
            .add_(1e-5)
            .sqrt()

...
class EntityDisambiguation:
    def __init__(self, base_url, wiki_version, user_config, reset_embeddings=False):
        ...
        self.emb = GenericLookup('entity_word_embedding', os.path.join(base_url, wiki_version, 'generated'))
        ...
        if os.path.exists(Path(self.config['model_path']).parent / 'lr_model.pkl'):
            with open(Path(self.config['model_path']).parent / 'lr_model.pkl', 'rb',) as f:
    ...
    def __get_config(self, user_config):
        ...
        if urlparse(str(config['model_path'])).scheme in ('http', 'https'):
            model_path = utils.fetch_model(config['model_path'], cache_dir=Path('~/.rel_cache').expanduser(),)
    ...
    def train(self, org_train_dataset, org_dev_datasets):
        ...
        for e in range(self.config['n_epochs']):
            ...
            for dc, batch in enumerate(train_dataset):  # each document is a minibatch
                ...
                token_ids, token_mask = utils.make_equal_len(token_ids, self.embeddings['word_voca'].unk_id)
                s_ltoken_ids, s_ltoken_mask = utils.make_equal_len(
                    s_ltoken_ids, self.embeddings['snd_voca'].unk_id, to_right=False
                )
                s_rtoken_ids, s_rtoken_mask = utils.make_equal_len(s_rtoken_ids, self.embeddings['snd_voca'].unk_id)
                s_rtoken_ids = [l[::-1] for l in s_rtoken_ids]
                s_rtoken_mask = [l[::-1] for l in s_rtoken_mask]
                s_mtoken_ids, s_mtoken_mask = utils.make_equal_len(s_mtoken_ids, self.embeddings['snd_voca'].unk_id)
                ...
                scores, ent_scores = self.model.forward(
                    token_ids, token_mask, entity_ids, entity_mask, p_e_m, self.embeddings, gold=true_pos.view(-1, 1),
                )
                ...
                total_loss += loss
                print(
                    'epoch', e, '%0.2f%%' % (dc / len(train_dataset) * 100), loss, end='\r',
                )
            ...
            if (e + 1) % eval_after_n_epochs == 0:
                ...
                for dname, data in dev_datasets:
                    ...
                    print(
                        dname, utils.tokgreen('Micro F1: {}, Recall: {}, Precision: {}'.format(f1, recall, precision)),
                    )

                    if dname == 'aida_testA':
                        dev_f1 = f1

                if self.config['learning_rate'] == 1e-4 and dev_f1 >= self.config['dev_f1_change_lr']:
    ...
    def __compute_confidence_legacy(self, scores, preds):
        ...
        for score, pred in zip(scores, preds):
            ...
            if not self.__max_conf:
                self.__max_conf = (self.config['keep_ctx_ent'] + self.config['keep_p_e_m'] - 1) * self.config['margin']
            ...
            if not eval_raw:
                ...
                for dname, entity in zip(doc_names, pred_entities):
                    if entity[0] != 'NIL':
                        ...
                        predictions[dname].append(
                            {'mention': entity[1], 'prediction': entity[0], 'candidates': entity[2], 'scores': [],}
                        )
    ...
    def prerank(self, dataset, dname, predict=False):
        ...
        for content in dataset:
            ...
            if self.config['keep_ctx_ent'] > 0:
                ...
                lctx_ids = [
                    m['context'][0][max(len(m['context'][0]) - self.config['prerank_ctx_window'] // 2, 0,) :]
                    for m in content
                ]
                rctx_ids = [
                    m['context'][1][: min(len(m['context'][1]), self.config['prerank_ctx_window'] // 2)]
                    for m in content
                ]
                ...
                log_probs = self.prerank_model.forward(token_ids, token_offsets, entity_ids, self.embeddings, self.emb)
    ...
    def __update_embeddings(self, emb_name, embs):
        ...
        if self.embeddings['{}_embeddings'.format(emb_name)]:
            new_weights = torch.cat((self.embeddings['{}_embeddings'.format(emb_name)].weight, embs))
        else:
            new_weights = embs

        # Weights are now updated, so we create a new Embedding layer.
        layer = torch.nn.Embedding(self.embeddings['{}_voca'.format(emb_name)].size(), self.config['emb_dims'])
        layer.weight = torch.nn.Parameter(new_weights)
        layer.grad = False
        self.embeddings['{}_embeddings'.format(emb_name)] = layer
        if emb_name == 'word':
            layer = torch.nn.EmbeddingBag(self.embeddings['{}_voca'.format(emb_name)].size(), self.config['emb_dims'],)
            layer.weight = torch.nn.Parameter(new_weights)
    ...
    def get_data_items(self, dataset, dname, predict=False):
        ...
        for doc_name, content in dataset.items():
            ...
            for m in content:
                ...
                named_cands = named_cands[: min(self.config['n_cands_before_rank'], len(named_cands))]

                # Candidate list per mention.
                named_cands_filt = set(
                    ['ENTITY/' + item for item in named_cands if item not in self.embeddings['entity_seen']]
                )

                self.__embed_words(named_cands_filt, 'entity', 'embeddings')

                # Use re.split() to make sure that special characters are considered.
                lctx = [x for x in re.split('(\W)', m['context'][0].strip()) if x != ' ']  # .split()
                rctx = [x for x in re.split('(\W)', m['context'][1].strip()) if x != ' ']  # split()

                words_filt = set([item for item in lctx + rctx if item not in self.embeddings['word_seen']])
                ...
                words_filt = set(
                    [item for item in snd_lctx + snd_rctx + snd_ment if item not in self.embeddings['snd_seen']]
                )
                ...
                if len(cands) == 0 and not predict:
                    ...
                    cands += [self.embeddings['entity_voca'].unk_id] * (self.config['n_cands_before_rank'] - len(cands))
                    named_cands += [Vocabulary.unk_token] * (self.config['n_cands_before_rank'] - len(named_cands))
                    p_e_m += [1e-8] * (self.config['n_cands_before_rank'] - len(p_e_m))
                    mask += [0.0] * (self.config['n_cands_before_rank'] - len(mask))

                lctx_ids = [self.embeddings['word_voca'].get_id(t) for t in lctx if utils.is_important_word(t)]
                ...
                rctx_ids = [self.embeddings['word_voca'].get_id(t) for t in rctx if utils.is_important_word(t)]
                ...
                ment_ids = [self.embeddings['word_voca'].get_id(t) for t in ment if utils.is_important_word(t)]
    ...
    def __load(self, path):
        ...
        if os.path.exists('{}.config'.format(path)):
            ...
            print('No configuration file found at {}, default settings will be used.'.format('{}.config'.format(path)))

        model = MulRelRanker(self.config, self.device).to(self.device)  # , self.embeddings

        if not torch.cuda.is_available():
            model.load_state_dict(
                torch.load('{}{}'.format(self.config['model_path'], '.state_dict'), map_location=torch.device('cpu'),)
            )
        else:
            model.load_state_dict(torch.load('{}{}'.format(self.config['model_path'], '.state_dict')))
        return model

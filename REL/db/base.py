...
class DB:
    ...
    def create_index(self, columns=None, table_name=None):
        ...
        createSecondaryIndex = 'CREATE INDEX if not exists idx_{} ON {}({})'.format('lower', 'wiki', 'lower')
        print(createSecondaryIndex)
        c.execute(createSecondaryIndex)
    ...
    def insert_batch_wiki(self, batch):
        ...
        """
        c = self.db.cursor()
        binarized = [(word, self.dict_to_binary(p_e_m), lower, occ) for word, p_e_m, lower, occ in batch]
        try:
            # Adding the transaction statement reduces total time from approx 37h to 1.3h.
            c.execute('BEGIN TRANSACTION;')
            c.executemany('insert into {} values (?, ?, ?, ?)'.format(self.table_name), binarized)
    ...
    def lookup(self, w, table_name, column='emb'):
        ...
        c.execute('BEGIN TRANSACTION;')
        for word in w:
            e = c.execute('select {} from {} where word = :word'.format(column, table_name), {'word': word},).fetchone()
    ...
    def lookup_wik(self, w, table_name, column):
        ...
        if column == 'lower':
            e = c.execute('select word from {} where {} = :word'.format(table_name, column), {'word': w},).fetchone()
        else:
            e = c.execute('select {} from {} where word = :word'.format(column, table_name), {'word': w},).fetchone()

...
class GenTrainingTest(MentionDetectionBase):
        ...
        for doc in dataset:
            ...
            for idx_sent, (sentence, ground_truth_sentence) in contents.items():
                for m, gt, start, ngram in ground_truth_sentence:
                    ...
                    left_ctxt, right_ctxt = self.get_ctxt(start, end, idx_sent, sentence, sentences_doc)
    ...
    def process_wned(self, dataset):
        ...
        for doc in root:
            ...
            if doc_name in exist_doc_names:
                print('Duplicate document found, will be removed later in the process: {}'.format(doc_name))
                continue
            ...
            for t in split_text:
                ...
                for _, _, pos, m in gt_sent:
                    assert m == t[pos : pos + len(m)], 'Wrong position mention {};{};{}'.format(m, pos, t)
                ...
                i += 1
                total_characters += len(t) + len(split)
            assert total_gt == total_assigned, 'We missed a ground truth.. {};{}'.format(total_gt, total_assigned)
                ...
                if '-DOCSTART-' in line:
                    if len(sentence) > 0:
                        ...
                        for gt in gt_sent:
                            ...
                            ), 'AIDA ground-truth incorrect position. {};{};{}'.format(sentence_words, gt[2], gt[3])
                    ...
                    if len(parts) in [7, 6] and parts[1] == 'B':
                        ...
                        gt_sent.append(
                            [self.preprocess_mention(mention_gt), ent_title, pos_mention_gt, mention_gt,]
                        )
                        words = mention_gt

                    if len(parts) >= 2 and parts[1] == 'B':
                        words = [modify_uppercase_phrase(x) for x in split_in_words_mention(parts[2])]
                    elif len(parts) >= 2 and parts[1] == 'I':
                        # Continuation of mention, which we have added prior
                        # to this iteration, so we skip it.
                        continue
                    else:
                        words = [modify_uppercase_phrase(w) for w in split_in_words_mention(parts[0])]  # WAS _mention
    ...
    def __save(self, mentions_dataset, file_name):
        ...
        with open(
            os.path.join(self.base_url, self.wiki_version, 'generated/test_train_data/', f'{file_name}.pkl',), 'wb',
        ) as f:
